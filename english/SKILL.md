...
Use this skill for whole-lecture or whole-topic explanation in English. Do not use it when the user asks for page-by-page translation, page-by-page explanation, or strict page-content correspondence; use `$lecture-pagewise-explainer-en` instead.

Write the explanation in English by default. For non-English source content, translate it into natural and accurate English. For English source content, preserve the original wording when reproducing it and explain it in English. For mixed-language content, translate non-English parts into English and preserve English parts. Do not switch the explanation to another language unless the user explicitly asks.
...
- Use this skill when the user explicitly asks in English for an overall explanation, holistic lesson, synthesis, conceptual storyline, connected knowledge path, or a complete and easy explanation that does not need strict page-by-page structure.
- Use `$lecture-pagewise-explainer-en` when the user explicitly asks for every page, page-by-page translation, page-by-page explanation, or strict page-number correspondence.
- If the user only says "explain this deck", "teach me this PDF", or otherwise leaves the mode ambiguous, ask them to choose between:
...
1. Identify the target file, requested scope, output language, and whether the user wants holistic or pagewise explanation. Ask only when one of these materially changes the task and remains ambiguous.
...
6. Determine the output form from the user's request:
   ...
   - for an ambiguous request for a "handout", prefer a file artifact and ask only if the required file type cannot be inferred.
...
8. Select only teaching-relevant visuals. Include diagrams, tables, code screenshots, maps, timelines, plots, layouts, or multi-step examples when they are necessary for understanding. Caption them in English and explain what the learner should notice.
...
For every knowledge point, state the core idea in plain English before adding detail. Then include enough explanation for a first-time learner to understand what the slides are teaching, why the material matters, how the pieces connect, and how the idea is applied.
...
If a topic has many details, separate "Core idea" from "Detailed explanation" or "Additional detail" so the learner can grasp the main point quickly without losing completeness.

### Page References And Coverage

Attach page references in parentheses to every concept, claim, source, formula, diagram, dataset, experiment, algorithm, code block, proof idea, mechanism, case, animation change, example, and exercise or problem, for example `Main theorem (Page 12)` or `Experimental results and limitations (Pages 18-20)`.
...
For each new concept, give its formal slide meaning, a plain English explanation, why it appears, how it supports later material, and its page reference.
...
```markdown
**Overall storyline**
Briefly state the problem addressed by the deck and how its knowledge path develops.

**1. Learning goals and necessary background**
...

**2. Core concepts and relationships**
...

**3. How the knowledge develops**
Organize by concepts and logic rather than translating page by page; retain page references for every covered item.

**4. Key reasoning / formulas / evidence / mechanisms / algorithms / systems / arguments**
Keep only the categories the lecture actually needs, and explain their logic completely.

**5. Examples / cases / experiments / exercises**
Reconstruct the important material and explain why it appears.

**6. Common confusions, scope, and limitations**
...
...
- Confirm non-English source content has been translated into English, while English source wording is preserved appropriately.
...
- For document artifacts, render and visually inspect the result for clipped text, overlap, missing images, unreadable text, broken captions, and excessive blank pages.
